...
var item  = function () {
	...
	this.suggestedNames = ko.observableArray([]);
	this.name.subscribe(function () {
		Sammy.log("completion subscriber");
		var self = this;
		self.suggestedNames([]);
		var url = #{jsAction @Invoices.getCompletions() /};
		...
		setTimeout(function () {
			$.post(url(), data, function(jsonResult) {
					if(jsonResult) {
						...
						self.suggestedNames(mappedItems);
					}
			});
		}.bind(this), 1000);
	}.bind(this));
...
ko.bindingHandlers.autocomplete = {
		update: function(element, list, allBindings) {
			Sammy.log("autocompletion hook: element " + element.name + ", list: " + list + ", allBindings().autocompleteText: " + allBindings().autocompleteText);
			var searchFor = allBindings().autocompleteText;
			var completionValues = ko.utils.arrayMap(list(), function (item) {
				return item[searchFor];
			});
			$(element).autocomplete({ source: completionValues });
